...
copyBuffers(slang::driver::Driver &driver,
            const std::shared_ptr<const slang::driver::Driver> &projectDriver,
...
VerilogDocument::VerilogDocument(
    ...
    std::shared_ptr<const slang::driver::Driver> projectDriver,
  ...
  for (auto &dir : projectIncludeDirectories) {
    if (!driver.sourceManager.addUserDirectories(dir)) {
      continue;
    }
  }
  ...
  if (projectDriver) {
    ...
    // Copy defines & undefines from project driver
    driver.options.defines.insert(driver.options.defines.end(),
                                  projectDriver->options.defines.begin(),
                                  projectDriver->options.defines.end());
    driver.options.undefines.insert(driver.options.undefines.end(),
                                    projectDriver->options.undefines.begin(),
                                    projectDriver->options.undefines.end());
    // Copy dependency buffers from project driver
    copyBuffers(driver, projectDriver, canonPath);
    // Reparse with new buffers
  }
